			...
			if (webdriver != null) {
				...
				for (Map<String, String> LanguageMap : langList) {
					try {
						...
						// String Maxbet= LanguageMap.get(Constant.Maxbet).trim();
						...
						try {
							 ...
							 * scfnlib.ClickByCoordinates("return " +
							 * cfnlib.XpathMap.get("ClicktoContinueCoordinatex"), "return " +
							 * cfnlib.XpathMap.get("ClicktoContinueCoordinatey"));
							 * imageLibrary.click("NFDButton"); Thread.sleep(2000);
							 * report.detailsAppendFolder("Verify able to click on NFD button",
							 * "Should be able to click on NFD button", "Clicked on NFD button", "Pass",
							 * languageCode);
							 * 
						...
						try {
							...
							if (cfnlib.isElementVisible("isMenuBtnVisible")) {
								...
								Thread.sleep(2000);
								/*
								 * report.detailsAppendFolder("Verify able to click on Menu button",
						...
						if (cfnlib.isElementVisible("isAutoplayMenuBtnVisible")) {
							...
							boolean istotalBetAutoplayCorrect = cfnlib.compareText(TotalBet, "gettotalBetAutoplayText",
									"yes");

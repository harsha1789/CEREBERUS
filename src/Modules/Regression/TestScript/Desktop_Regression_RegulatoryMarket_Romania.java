...
public class Desktop_Regression_RegulatoryMarket_Romania
{
	 ...
	 * =======================================================================
	   1.Launch the Romania with EN  & ro
	...
	Logger log = Logger.getLogger(Desktop_Regression_RegulatoryMarket_Romania.class.getName());
	public void script() throws Exception
	{
		...
		int mintStepNo=0;
		String userName="" ;
		String regMarket = "Romania"; 
		int envid = 1658;
		int productId = 5543;
		String currencyIsoCode = "EUR";
		int marketTypeID = 27;
		double balance = 10000;
		String lobbyname= "romaniaqf";
		String languages[]= {"en","ro"};
		
		Desktop_HTML_Report romania = new Desktop_HTML_Report(webdriver, browserName, filePath, startTime, mstrTCName,mstrTCDesc, mstrModuleName, mintDetailCount, mintPassed, mintFailed, mintWarnings, mintStepNo, status,gameName);
		DesktopFunctionLibraryFactory desktopFunctionLibraryFactory=new DesktopFunctionLibraryFactory();
		CFNLibrary_Desktop cfnlib=desktopFunctionLibraryFactory.getFunctionLibrary(framework, webdriver, proxy,romania, gameName);
			...
			if (webdriver != null) 
			{
               ...
               // String LaunchURl = url.replaceFirst("\\busername=.*?(&|$)", "externalToken="+userName+"$1");
				...
				else
				{
			      ...
			      // Currency Check
					boolean curencycheck = cfnlib.romaniaCurrencyCheck(romania);
					if (curencycheck)
					{
				...
				*/	
					...
					boolean clock = cfnlib.clockFromTopBar(romania);
					if (clock )
					{
					...
					if (isstopbtn) 
					{
						romania.detailsAppend("Stop button", "Stop button is Available", "Stop button is Available","PASS");
					}
					else 
					{
						romania.detailsAppend("Stop button", "Stop button is not Available", "Stop button is not  Available","FAIL");
					}
					Thread.sleep(2000);
					...
					if (isquickspin) 
					{
						romania.detailsAppend("Quickspin ", "Quickspin is  Available ", "Quickspin is Available", "PASS");
					}
					else
					{
						romania.detailsAppend("Quickspin ", "Quickspin is not  Available ", "Quickspin is not Available", "FAIL");
					}
					Thread.sleep(2000);
					...
					if (menuopen) 
					{
						...
						if (closemenu)
						{
							...
							romania.detailsAppend("Menu Closed", "Menu is Closed ", "Menu is Closed", "PASS");
						} 
						else
						{
							log.debug("Menu Closed");
							romania.detailsAppend("Menu Closed", "Menu is Closed ", "Menu is Closed", "FAIL");
						}
						Thread.sleep(2000);
					} 
					else
					...
					boolean help = cfnlib.romaniaHelpFromTopBarMenu(romania);
					if (help) 
					{
						romania.detailsAppend("  Help from Menu ", "Help is Displayed and its Navigation", "Help is Displayed and its Navigation", "PASS");															
					}
					else 
					{
						romania.detailsAppend("  Help from Menu ", "Help is Displayed and its Navigation", "Help is Displayed and its Navigation", "FAIL");	
					}
					Thread.sleep(2000);
					
	
					// Verify  Game Hitory from Menu
					boolean gameHistoryFromMenu = cfnlib.romaniaGameHistoryFromTopBarMenu(romania);
					if (gameHistoryFromMenu) 
					{
						romania.detailsAppend("Game History from Menu ", " Game History is Displayed and its Navigation", "Game History is Displayed and its Navigation", "PASS");
						
					}
					else 
					{
						romania.detailsAppend("Game History from Menu ", " Game History is Displayed and its Navigation", "Game History is Displayed and its Navigation", "FAIL");
					}

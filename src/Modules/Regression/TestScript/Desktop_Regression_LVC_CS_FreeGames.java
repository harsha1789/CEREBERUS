 ...
 *
 */
public class Desktop_Regression_LVC_CS_FreeGames
{

	public static Logger log = Logger.getLogger(Desktop_Regression_LVC_CS_FreeGames.class.getName());
	...
	public void script() throws Exception 
	{
		...
		Desktop_HTML_Report lvcReport = new Desktop_HTML_Report(webdriver, browserName, filePath, startTime,
		...
		try 
		{
			...
			if (webdriver != null) 
			{
				String strFileName = TestPropReader.getInstance().getProperty("FreeGamesTDPath");
				...
				for (Map<String, String> currencyMap : currencyList) 
				{
							...
							if (util.copyFilesToTestServer(mid, cid, testDataFile, userName, copiedFiles, currencyID,isoCode))
							{
								...
								if (isFreeGameAssigned) 
								{
									...
									if (isGameLoaded) 
									{
										...
										//method is to verify the freegames Entry Screen Buttons
										...
										// Click on info, get text and verify currency
										...
										if (isfreeGameEntryInfoVisible)
										{
											...
											lvcReport.detailsAppendFolder("Free Games"," Entry Screen Info Text Validation"," Entry Screen Info Text Validation", "PASS", currencyName);
										}
										else
										{
										...
										boolean playNowButtonAvailability = cfnlib.func_Click("playNow");
										...
										else
										{
											...
											lvcReport.detailsAppendFolder("Free Game", " Big Win Amt", "Big Win Amt", "FAIL",currencyName);
										}Thread.sleep(2000); 
										
										// free games on refresh , Resume and Info Text currency validation
										boolean freeGamesOnRefresh =cfnlib.verifyRegularExpression(lvcReport,regExpr,cfnlib.freeGameOnRefresh(lvcReport,currencyName));
										if(freeGamesOnRefresh )
										{
											System.out.println("Free Game On Refresh : PASS");log.debug("Free Game On Refresh : PASS");
											lvcReport.detailsAppendFolder("Free Game", "On Refresh Info Text Validation and back to Basegame ", "On Refresh Info Text Validation and back to Basegame ", "PASS",currencyName);
											Thread.sleep(10000);
											if(cfnlib.XpathMap.get("TestDataRepeat").equalsIgnoreCase("yes"))
											{
											// To verify Big win  
											cfnlib.spinclick();Thread.sleep(6000);
											}
										}
										else
										{
											System.out.println("Free Game On Refresh : FAIL");log.debug("Free Game On Refresh : FAIL");
											lvcReport.detailsAppendFolder("Free Game", "On Refresh Info Text Validation and back to Basegame ", "On Refresh Info Text Validation and back to Basegame ", "FAIL",currencyName);
										}Thread.sleep(2000);
										...
										if(winFormatVerification)
										{
											...
											lvcReport.detailsAppendFolder("Free Game", "Win Amt", "Win Amt", "PASS",currencyName);
										}
										else
										{
											System.out.println("Free Game Win Value : FAIL");log.debug("Free Game Win Value : FAIL");
											lvcReport.detailsAppendFolder("Free Game", "Win Amt", "Win Amt", "FAIL",currencyName);
										}Thread.sleep(3000);
										...
										if (cfnlib.checkAvilabilityofElement("BonusFeatureImage"))
										{
										 ...
										 //method is used to get the click , get the bonus text,verifies bonus summary screen in base game and check the currency format
										  cfnlib.verifyRegularExpressionUsingArrays(lvcReport,regExprNoSymbol,cfnlib.bonusFeatureClickandGetText(lvcReport,currencyName));
										  System.out.println("Bonus Game : PASS");log.debug("Bonus Game : PASS"); Thread.sleep(3000);
										  cfnlib.spinclick();
										  Thread.sleep(2000);
										}
										else
								...
								{
									log.error("Skipping the execution as free games assignment is failed");
									lvcReport.detailsAppend("Skipping the execution as free games assignment is failed ", " ", "", "FAIL");
								}
					...
					}//closing catch exception 
				} // closing for loop of mapping 
			}//closing if loop
			
			else
			{
				lvcReport.detailsAppend("Unable to load the Web Driver ", " ", "", "FAIL");
			}
		}//closing try block 
